...
analogWrite(M1, 0); // O motor 1 LIGA
analogWrite(M2, 210); // O motor 2 fica ligado, fazendo assim o carrinho virar
delay(1500);

analogWrite(M1, 255); // O motor 1 DELSLIGA
analogWrite(M2, 210); // O motor 2 fica ligado, fazendo assim o carrinho virar
delay(800);
...
analogWrite(M1, 0); // O motor 1 LIGA
analogWrite(M2, 0); // O motor DESLIGA
delay(600);
...
analogWrite(M1, 255); // O motor 1 DESLIGA
analogWrite(M2, 0); // O motor 2 DESLIGA
delay(2400);


// Nota ao professor: O outro código está abaixo, já que o envio não permite enviar mais de um código
// //Aqui está toda a lógica de comportamento do robô: Para a cor branca atribuímos o valor 0 e, para a cor preta, o valor 1.
// if((Sensor1 == 1) && (Sensor2 == 1)){ // Se detectar na extremidade das faixas duas cores brancas
// analogWrite(M1, 0); // Ambos motores ligam na mesma velocidade
// analogWrite(M2, 255);
// }

// if((Sensor1 == 0) && (Sensor2 == 1)){ // Se detectar um lado preto e o outro branco
// analogWrite(M1, 255); // O motor 1 desliga
// analogWrite(M2, 255); // O motor 2 fica ligado, fazendo assim o carrinho virar
// }
